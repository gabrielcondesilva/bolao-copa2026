      ...
      <form action={action} className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <TeamSelect
            ...
            defaultValue={existing?.champion_team_id ?? ''}
          ...
          <TeamSelect
            ...
            teamsByGroup={teamsByGroup}
            defaultValue={existing?.runner_up_team_id ?? ''}
            disabled={isLocked || pending}
          />
          <TeamSelect
            label="🥉 3º Colocado"
            name="third_team_id"
            teamsByGroup={teamsByGroup}
            defaultValue={existing?.third_team_id ?? ''}
            disabled={isLocked || pending}
          />
          <TeamSelect
            label="4º Colocado"
            name="fourth_team_id"
            teamsByGroup={teamsByGroup}
            defaultValue={existing?.fourth_team_id ?? ''}
            disabled={isLocked || pending}
          />
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <TextInput
            label="Artilheiro"
            name="top_scorer"
            defaultValue={existing?.top_scorer ?? ''}
            disabled={isLocked || pending}
          />
          <TextInput
            label="Craque do Torneio"
            name="best_player"
            defaultValue={existing?.best_player ?? ''}
            disabled={isLocked || pending}
          />
        </div>
