...
export function KnockoutPalpites({ matches, palpites, teams, deadlineAt, exceptionUntil }: Props) {
  ...
  const missingIds = useMemo(() => {
    ...
    return new Set(playableMatches.filter(m => !isValid(states[m.id])).map(m => m.id))
  }, [showErrors, states, playableMatches])

  async function handleSaveAll() {
    const missing = playableMatches.filter(m => !isValid(states[m.id]))
    if (missing.length > 0) {
      setShowErrors(true)
      return
    }
    ...
    const items = playableMatches.map(m => ({
      matchId: m.id,
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        ...
        {!isEditing && filledCount > 0 && (
          <span className="text-sm font-medium text-green-600">
            {filledCount === playableMatches.length
              ? '✓ Todos os palpites salvos'
              : `✓ ${filledCount} de ${playableMatches.length} salvos`}
          </span>
        )}
      </div>
    </div>
